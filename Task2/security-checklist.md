...
На основе анализа архитектуры системы PropDevelopment и выявленных проблем безопасности выбраны следующие разделы проверочного листа:

1. **Управление доступом** - критично из-за проблем с неавторизованным доступом к административным панелям и отображением чужих данных клиентам
2. **Безопасность данных** - необходимо для соответствия 152-ФЗ и предотвращения штрафов до 18 млн руб
3. **Защита инфраструктуры** - важно для защиты Kubernetes кластера и API сервисов
4. **Управление инцидентами** - поможет выявить и предотвратить утечки данных
5. **Специфические аспекты** - мобильные устройства, облачные сервисы, физическая безопасность

PropDevelopment - крупная строительная компания (топ-3), работающая с персональными данными клиентов и собственников. Множественные системы включают frontend, backend API, базы данных, интеграции с партнерами. Высокие риски связаны с финансовыми данными, персональными данными, соответствием 152-ФЗ. Kubernetes инфраструктура требует специальных мер безопасности для контейнеризованных приложений.
...
| 1 | Используется ли многофакторная аутентификация (MFA) для доступа к критическим системам и сервисам? | Нет | Отсутствует MFA для административных панелей и Kubernetes кластера |
| 2 | Настроены ли требования к сложности паролей и их регулярное обновление? | Нет | Политика паролей не настроена, используются простые пароли |
...
| 5 | Выполняется ли регулярное тестирование процесса восстановления данных из резервных копий? | Нет | Нет тестирования восстановления из резервных копий |
